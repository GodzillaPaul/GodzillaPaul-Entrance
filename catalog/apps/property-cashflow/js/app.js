(function(){
  ...
  var MONTHLY_FUND_SOURCE=window.GP_MONTHLY_DIVIDEND;
  var MARKET_CSV_URL=MONTHLY_FUND_SOURCE.googleSheetCsvUrl;
  var FUND_MARKET_API=MONTHLY_FUND_SOURCE.fundMarketApi;
  var market={fxRate:MONTHLY_FUND_SOURCE.fxRate,fxDate:MONTHLY_FUND_SOURCE.fxDate,audRate:22.4251,audDate:'2026-05-27',funds:MONTHLY_FUND_SOURCE.cloneFunds()};
  ...
  function signedMoney(n){var v=Math.round(n); return (v>=0?'+$':'-$')+Math.abs(v).toLocaleString('en-US')}
  function setFlow(id,n){
    var el=$(id); if(!el) return;
    setText(id,signedMoney(n));
    var ok=n>=0;
    el.classList.toggle('flow-pos',ok); el.classList.toggle('flow-neg',!ok);
    el.classList.toggle('green',ok); el.classList.toggle('red',!ok);
    el.classList.toggle('is-positive',ok); el.classList.toggle('is-negative',!ok);
  }
  ...
  function refreshOfficialFundMarket(){
    ...
    var codes=MONTHLY_FUND_SOURCE.officialCodes.join(',');
    return fetch(FUND_MARKET_API+'?codes='+encodeURIComponent(codes)+'&cacheBust='+Date.now(),{cache:'no-store'}).then(function(res){if(!res.ok)throw new Error('HTTP '+res.status);return res.json()}).then(applyFundMarketPayload);
  }
  ...
  function renderQuickPlan(prefix,plan,rows,totalPay){
    ...
    var netFlow=plan.monthlyIncome-(totalPay||0)-plan.policyCost;
    ...
    rows.forEach(function(r){
      ...
      var ratio=r.balance>0?(r.surr/r.balance*100):999;
      var fill=Math.max(Math.min(ratio,100),0);
      var ratioLabel=r.balance>0?Math.round(ratio)+'%':'已還清';
      tr.innerHTML='<td>第 '+r.year+' 年</td><td>'+money(r.balance)+'</td><td>'+money(r.surr)+'</td>'+
        '<td class="meter-cell"><span class="meter" role="img" aria-label="覆蓋率 '+ratioLabel+'"><i class="'+(isCovered?'ok':'')+'" style="width:'+fill.toFixed(1)+'%"></i></span><b class="'+(isCovered?'good-cell':'muted-cell')+'">'+ratioLabel+'</b></td>'+
        '<td>'+money(r.cumCash)+'</td><td>'+money(r.redeem)+'</td>';
      body.appendChild(tr);
    });
  }

  var quickPlanCache={};
  function quickTone(payoffYear,netFlow){
  ...
  function renderQuickMilestones(plan){
    var host=$('q-table-conclusion'); if(!host) return;
    if(!plan){host.textContent='尚未試算'; return;}
    var src=$(plan.kind==='p1'?'q-p1-conclusion':'q-p2-conclusion');
    host.textContent=src?src.textContent:'尚未試算';
    host.className='table-conclusion '+(src?(src.className.replace('plan-conclusion','').trim()||'is-warn'):'is-warn');
  }
  function renderQuickTable(){
    var source=$(qSelectedPlan==='p1'?'q-p1-report':'q-p2-report'), target=$('q-active-report');
    if(!source||!target) return;
    target.innerHTML=source.innerHTML;
    renderQuickMilestones(quickPlanCache[qSelectedPlan]);
    ...
    var span=(quickPlanCache[qSelectedPlan]&&quickPlanCache[qSelectedPlan].rows.length)||20;
    setText('q-table-summary',payoff==='尚未達成'?planName+'在 '+span+' 年貸款年期內，PFK 解約金尚未超過剩餘房貸，可提高可增貸金額或調整配置再比較。':planName+'推估 '+payoff+' 的 PFK 解約金會超過剩餘房貸，具備一次結清的觀察點；明細已列到房貸餘額歸零的第 '+span+' 年。');
    var foldLabel=document.querySelector('#q-table-fold summary span');
    if(foldLabel) foldLabel.textContent='完整 '+span+' 年明細';
  }
  ...
  function buildQuickPlan(kind,budgetWan,totalLoanWan,loanRows,horizonYears){
    ...
    var span=Math.max(Math.round(horizonYears||20),1);
    if(pfk.rows&&pfk.rows.length) span=Math.min(span,pfk.rows.length);
  ...
  function calcQ(){
    ...
    var loanYears=clamp(Math.round(num('q-extra-years'))||20,1,40);
    var totalPay=qMethod==='equal'?pay(totalLoan,num('q-extra-rate'),loanYears):interestOnly(totalLoan,num('q-extra-rate'));
    var loanRows=loanBalanceRows(totalLoan,num('q-extra-rate'),loanYears,qMethod,loanYears);
    var p1=buildQuickPlan('p1',budget,totalLoan,loanRows,loanYears), p2=buildQuickPlan('p2',budget,totalLoan,loanRows,loanYears);
    var p1Net=p1.monthlyIncome-totalPay-p1.policyCost, p2Net=p2.monthlyIncome-totalPay-p2.policyCost;
    // 先比結清年度，同年度再比每月現金流，讓結論卡與摘要條永遠指向同一個方案
    var best;
    if(p1.payoffYear&&p2.payoffYear) best=(p1.payoffYear<p2.payoffYear)?p1:((p2.payoffYear<p1.payoffYear)?p2:(p2Net>=p1Net?p2:p1));
    else if(p1.payoffYear) best=p1;
    else if(p2.payoffYear) best=p2;
    else best=null;
    var lead=best||(p2Net>=p1Net?p2:p1);
    var leadNet=lead.kind==='p1'?p1Net:p2Net;
    var leadName=lead.kind==='p1'?'方案一':'方案二';
    ...
    setFlow('q-p1-live-netflow',p1Net);
    setFlow('q-p2-live-netflow',p2Net);
    setFlow('q-safety-p1-netflow',p1Net);
    setFlow('q-safety-p2-netflow',p2Net);
    setFlow('q-result-netflow',leadNet);
    setText('q-result-netflow-label',leadName+'每月現金流');
    ...
    quickPlanCache={p1:p1,p2:p2};
    renderQuickPlan('q-p1',p1,p1.rows,totalPay); renderQuickPlan('q-p2',p2,p2.rows,totalPay);
    setText('q-payoff-strip-copy',best?'目前試算以 '+(best.kind==='p1'?'方案一':'方案二')+' 較早讓 PFK 解約金超過剩餘房貸。':loanYears+' 年內 PFK 解約金尚未超過剩餘房貸。');
    setText('q-payoff-copy',best?'目前試算以 '+(best.kind==='p1'?'方案一':'方案二')+' 較早達標，PFK 解約金約可在第 '+best.payoffYear+' 年超過剩餘房貸。':loanYears+' 年貸款年期內 PFK 解約金尚未超過剩餘房貸，可提高可增貸金額、拉長配息累積或調整標的再試算。');
    renderQuickTable();
  }
  ...
  function reportValue(id, unit){
    var el=$(id);
    if(!el) return '—';
    var v = el.value != null ? String(el.value).trim() : String(el.textContent||'').trim();
    return v ? v + (unit||'') : '—';
  }
  function esc(s){
  ...
  function reportMetric(label,value,tone){
    return '<div class="print-metric '+(tone||'')+'"><span>'+esc(label)+'</span><b>'+esc(value)+'</b></div>';
  }
  function reportLine(label,value){
    return '<div class="print-line"><span>'+esc(label)+'</span><b>'+esc(value)+'</b></div>';
  }
  function reportCard(title,lines,klass){
    return '<section class="print-card '+(klass||'')+'"><h3>'+esc(title)+'</h3><div class="print-lines">'+lines.map(function(x){return reportLine(x[0],x[1]);}).join('')+'</div></section>';
  }
  function reportSection(title,html){
    return '<section class="print-section"><h2>'+esc(title)+'</h2>'+html+'</section>';
  }
  function reportNote(title,copy){
    return '<section class="print-note"><strong>'+esc(title)+'</strong><p>'+esc(copy)+'</p></section>';
  }
  function reportHeader(title,subtitle,headline){
    var today=new Date().toLocaleDateString('zh-TW',{year:'numeric',month:'2-digit',day:'2-digit'});
    return '<header class="print-header">'+
      '<div><div class="print-kicker">PROPERTY CASHFLOW OS</div><h1>'+esc(title)+'</h1><p>'+esc(subtitle)+'</p></div>'+
      '<div class="print-date">產出日期<br><b>'+esc(today)+'</b></div>'+
      '</header><section class="print-hero"><small>核心結論</small><strong>'+esc(headline)+'</strong></section>';
  }
  function reportTableFrom(id, limit){
    var el=$(id);
    if(!el) return '';
    var table=el.tagName==='TBODY' ? el.closest('table') : el;
    var head=Array.from((table||el).querySelectorAll('thead th')).map(function(th){return '<th>'+esc(th.textContent)+'</th>';}).join('');
    var trs=el.tagName==='TBODY' ? Array.from(el.children) : Array.from(el.querySelectorAll('tbody tr'));
    if(limit && trs.length>limit){
      var first=trs.slice(0,Math.max(limit-1,1));
      var last=trs.slice(-1);
      trs=first.concat(last);
    }
    var body=trs.map(function(tr){
      return '<tr>'+Array.from(tr.children).map(function(td){return '<td>'+esc(td.textContent)+'</td>';}).join('')+'</tr>';
    }).join('');
    return '<div class="print-table-wrap"><table class="print-table"><thead><tr>'+head+'</tr></thead><tbody>'+body+'</tbody></table></div>';
  }
  function buildToolAReport(){
    setToolAStep('result', false); calcA();
    var html=reportHeader('房貸減壓方案','比較方案一與方案二，確認活化後每月房貸是否真的下降。',reportText('a-client-headline'));
    html+= '<div class="print-metrics">'+[
      reportMetric('現在月付',reportText('a-lens-now')),
      reportMetric('活化後月付',reportText('a-lens-after'),'good'),
      reportMetric('每月減壓',reportText('a-lens-save'),'gold'),
      reportMetric('壽險額度',reportText('a-policy-sum')),
      reportMetric('保留現金',reportText('a-reserve-show'))
    ].join('')+'</div>';
    html+= reportSection('試算條件','<div class="print-facts">'+[
      reportLine('目前房貸金額',reportValue('a-current-loan','萬元')),
      reportLine('現行利率',reportValue('a-current-rate','%')),
      reportLine('剩餘年期',reportValue('a-current-years','年')),
      reportLine('活化資金',reportValue('a-extra','萬元')),
      reportLine('新增資金利率',reportValue('a-extra-rate','%')),
      reportLine('壽險保障目標',reportValue('a-cover','萬元')),
      reportLine('美元匯率',reportValue('a-fx',''))
    ].join('')+'</div>');
    html+= reportSection('方案比較','<div class="print-card-grid">'+
      reportCard('方案一：先扣兩年保費',[
        ['可投入金額',reportText('a-p1-invest')],
        ['PFK兩年保費',reportText('a-p1-life-cost')],
        ['月配息',reportText('a-income-p1')],
        ['每月房貸',reportText('a-p1-final-mortgage')],
        ['每月減壓',reportText('a-p1-relief')]
      ],'')+
      reportCard('方案二：首年保費先扣，第二年保單借款',[
        ['可投入金額',reportText('a-p2-invest')],
        ['PFK首年保費',reportText('a-p2-life-cost')],
        ['月配息',reportText('a-income-p2')],
        ['第二年保費借款月息',reportText('a-policy-cost')],
        ['每月房貸',reportText('a-p2-final-mortgage')],
        ['每月減壓',reportText('a-p2-relief')]
      ],'highlight')+'</div>');
    html+= reportNote('系統建議',reportText('a-judge')+' '+reportText('a-decision-copy'));
    html+= reportNote('顧問備註',reportText('a-plan-copy'));
    return html;
  }
  function buildToolBReport(){
    setToolBStep('result', false); calcB();
    var html=reportHeader('以房養老現金流規劃','把房屋價值、安全活化比例與月現金流放在同一張 A4 報告。',reportText('b-client-headline'));
    html+= '<div class="print-metrics">'+[
      reportMetric('預估每月總配息流',reportText('b-cover-cashflow'),'gold'),
      reportMetric('融資額度',reportText('b-required')),
      reportMetric('安全最大活化空間',reportText('b-available')),
      reportMetric('融資月成本',reportText('b-loan-pay'),'bad'),
      reportMetric('安全純月淨現金流',reportText('b-net'),reportText('b-net').indexOf('-')===0?'bad':'good')
    ].join('')+'</div>';
    html+= reportSection('試算條件','<div class="print-facts">'+[
      reportLine('房屋市值',reportValue('b-home-value','萬元')),
      reportLine('現有房貸',reportValue('b-current-loan','萬元')),
      reportLine('可用安全成數',reportValue('b-ltv','%')),
      reportLine('本次活化金額',reportValue('b-active-amount','萬元')),
      reportLine('融資利率',reportValue('b-rate','%')),
      reportLine('保留現金',reportValue('b-reserve','萬元')),
      reportLine('壽險保障目標',reportValue('b-cover','萬元'))
    ].join('')+'</div>');
    html+= reportSection('方案比較','<div class="print-card-grid">'+
      reportCard('方案一：先扣兩年壽險成本',[
        ['淨現金流',reportText('b-p1-net')],
        ['月配息流入',reportText('b-p1-income')],
        ['房貸成本',reportText('b-p1-loan')],
        ['壽險兩年保費',reportText('b-p1-premium')]
      ],'')+
      reportCard('方案二：首年預留，第二年保單借款',[
        ['淨現金流',reportText('b-planned-net')],
        ['月配息流入',reportText('b-planned-income')],
        ['房貸成本',reportText('b-planned-loan')],
        ['保費借款月息',reportText('b-planned-life')]
      ],'highlight')+'</div>');
    html+= reportSection('風險檢查','<div class="print-metrics compact">'+[
      reportMetric('安全空間使用率',reportText('b-space-pct')),
      reportMetric('保留現金安全月數',reportText('b-reserve-months')),
      reportMetric('利率上升1%後月壓力',reportText('b-stress-rate'),'bad'),
      reportMetric('壽險防護覆蓋率',reportText('b-cover-rate'))
    ].join('')+'</div>');
    html+= reportNote(reportText('b-decision'),reportText('b-decision-copy'));
    return html;
  }
  function buildToolDReport(){
    setToolDStep('result', false); calcD();
    var html=reportHeader('傳承安家方案','比較穩健預留與效率借款兩種壽險預算路徑。',reportText('d-client-headline'));
    html+= '<div class="print-metrics">'+[
      reportMetric('月配息流入',reportText('d-lens-income'),'gold'),
      reportMetric('月成本合計',reportText('d-lens-cost'),'bad'),
      reportMetric('第6年月套利',reportText('d-lens-net'),reportText('d-lens-net').indexOf('-')===0?'bad':'good'),
      reportMetric('第6年身故金',reportText('d-planned-cover')),
      reportMetric('壽險額度',reportText('d-planned-sum'))
    ].join('')+'</div>';
    html+= reportSection('試算條件','<div class="print-facts">'+[
      reportLine('房屋市值',reportValue('d-home-value','萬元')),
      reportLine('本次活化金額',reportValue('d-active-amount','萬元')),
      reportLine('融資利率',reportValue('d-rate','%')),
      reportLine('保留現金',reportValue('d-reserve','萬元')),
      reportLine('壽險年預算',reportText('d-planned-annual')),
      reportLine('美元匯率',reportValue('d-fx',''))
    ].join('')+'</div>');
    html+= reportSection('方案比較','<div class="print-card-grid">'+
      reportCard('方案一：先預留 6 年壽險預算',[
        ['月套利',reportText('d-p1-net')],
        ['投入月配息本金',reportText('d-p1-invest')],
        ['6年壽險預算',reportText('d-p1-premium')],
        ['房貸成本',reportText('d-p1-loan')]
      ],'')+
      reportCard('方案二：首年預留，後 5 年保單借款',[
        ['第6年月套利',reportText('d-planned-net')],
        ['投入月配息本金',reportText('d-p2-invest')],
        ['第6年保單借款月息',reportText('d-planned-life')],
        ['房貸成本',reportText('d-planned-loan')]
      ],'highlight')+'</div>');
    html+= reportSection('6 年逐年套利表',reportTableFrom('d-pfj-flow-body',0).replace('<thead><tr></tr></thead>','<thead><tr><th>年度</th><th>月配息總額</th><th>借款金額</th><th>房貸成本</th><th>借款月息</th><th>套利</th></tr></thead>'));
    html+= reportNote('系統判讀',reportText('d-client-sub'));
    return html;
  }
  function buildCalculatorReport(){
    calcC();
    var html=reportHeader('房貸成本計算機','直式 A4 攤還摘要，快速掌握月付、總利息與年度明細。',reportText('c-primary-label')+' '+reportText('c-primary'));
    html+= '<div class="print-metrics">'+[
      reportMetric(reportText('c-primary-label'),reportText('c-primary'),'gold'),
      reportMetric('總累計利息支出',reportText('c-interest')),
      reportMetric('總支付款 / 到期償本',reportText('c-total')),
      reportMetric('首月實質還本金額',reportText('c-first-principal')),
      reportMetric('寬限期月利息支出',reportText('c-grace-pay'))
    ].join('')+'</div>';
    html+= reportSection('貸款條件','<div class="print-facts">'+[
      reportLine('貸款總額度',reportValue('c-amount','萬元')),
      reportLine('實質動用額度',reportValue('c-drawn','萬元')),
      reportLine('核定年利率',reportValue('c-rate','%')),
      reportLine('貸款總年期',reportValue('c-years','年')),
      reportLine('寬限期設定',reportValue('c-grace','年'))
    ].join('')+'</div>');
    html+= reportNote('計算說明',reportText('c-caption'));
  ...
  function buildQuickReport(){
    ...
    html+= '<div class="print-metrics">'+[
      ...
      reportMetric('增貸月付',reportText('q-result-total-pay')),
      reportMetric('方案一每月現金流',reportText('q-p1-netflow')),
      reportMetric('方案二每月現金流',reportText('q-p2-netflow'),'good')
    ].join('')+'</div>';
    html+= reportSection('試算條件','<div class="print-facts">'+[
    ...
    html+= reportSection('方案比較','<div class="print-card-grid">'+
      reportCard('方案一：60% 配息，40% PFK兩年總預算',[
        ...
        ['每月淨現金流（結清前）',reportText('q-p1-netflow')],
        ['PFK解約金超過剩餘房貸',reportText('q-p1-payoff')],
        ['結論',reportText('q-p1-conclusion')]
      ],'')+
      reportCard('方案二：80% 配息，20% 首年PFK',[
        ...
        ['每月淨現金流（結清前）',reportText('q-p2-netflow')],
        ['PFK解約金超過剩餘房貸',reportText('q-p2-payoff')],
        ['結論',reportText('q-p2-conclusion')]
      ],'highlight')+'</div>');
    html+= reportSection('方案一年度報表',reportTableFrom('q-p1-report',21).replace('<thead><tr></tr></thead>','<thead><tr><th>年期</th><th>剩餘房貸</th><th>PFK解約金</th><th>覆蓋率</th><th>累積配息還款</th><th>可結清差額</th></tr></thead>'));
    html+= reportSection('方案二年度報表',reportTableFrom('q-p2-report',21).replace('<thead><tr></tr></thead>','<thead><tr><th>年期</th><th>剩餘房貸</th><th>PFK解約金</th><th>覆蓋率</th><th>累積配息還款</th><th>可結清差額</th></tr></thead>'));
  ...
  var qc=$('q-copy-summary'); if(qc){qc.onclick=function(){var t=function(id){return (($(id)||{}).textContent||'').trim()}; copyToClipboard('房產月收引擎【快速試算】\n可增貸金額：'+t('q-result-total-loan')+'\n增貸月付：'+t('q-result-total-pay')+'\n方案一 每月淨現金流：'+t('q-p1-netflow')+'\n  '+t('q-p1-conclusion')+'\n方案二 每月淨現金流：'+t('q-p2-netflow')+'\n  '+t('q-p2-conclusion')+'\n判讀：'+t('q-payoff-copy'))}}
