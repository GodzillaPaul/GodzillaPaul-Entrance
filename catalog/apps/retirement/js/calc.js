...
function calcActualAge(by, bm, bd) {
  const now = new Date();
...
const CHILD_TOTAL_SUM_ASSURED_CAP = 20000000;
const CHILD_TARGET_PREMIUM_MIN = 7500;
const CHILD_TARGET_PREMIUM_MAX = 15000;
...
function calcMaxSA(target, age, sex, actualAge) {
  const [lo, hi] = getMultTable(sex)[Math.min(age, 64)] || [0, 0];
  const rounded = lo === hi
    ? Math.ceil(target * hi / 1000) * 1000
    : Math.floor(target * hi / 1000) * 1000;
  const productCap = 60000000;
  const childCap = actualAge < 15 ? CHILD_TOTAL_SUM_ASSURED_CAP : productCap;
  return Math.min(productCap, childCap, rounded);
}
...
function runCalc() {
  ...
  // Validation
  if (isChild) {
    if (target < CHILD_TARGET_PREMIUM_MIN) return { error: `未滿 15 足歲年繳目標保費下限 ${fmt(CHILD_TARGET_PREMIUM_MIN)} 元` };
    if (target > CHILD_TARGET_PREMIUM_MAX) return { error: `未滿 15 足歲年繳目標保費上限 ${fmt(CHILD_TARGET_PREMIUM_MAX)} 元` };
    if (Math.round(target) % 100 !== 0) return { error: '年繳目標保費須為百元整數' };
  } else {
  ...
  const maxSA  = calcMaxSA(target, iAge, sex, actualAge);
  if (minSA > maxSA) {
    return {
      error: actualAge < 15
        ? `年繳保費過高：建議保額下限 ${fmt(minSA)} 已超過未滿 15 足歲累計總保額上限 ${fmt(maxSA)}`
        : `年繳保費過高：建議保額下限 ${fmt(minSA)} 已超過最高可投保保額 ${fmt(maxSA)}`
    };
  }
  ...
  if (SA > maxSA) {
    return { error: `保額超過最高可投保保額 ${fmt(maxSA)} 元，請調低保額` };
  }
  ...
  return { rows, iAge, actualAge, minSA, maxSA, SA, target, highDisc, lo_, hi_, avg_, stages, premiumStopYear, payLimitLabel };
}
